...
def do_ping2(e):
    msg = ''
    print(f"{e}")
    result = check_equipo2(e)
    print(f"PING2 RESULT {result}")
    if result:
        msg += result
    # if len(msg.strip()) <= 1:
    #     print("sin cambios")
    ...
    try:
        ...
        if ping(ip):
            if 'state' not in e or e["state"] != True:
                message = f"{name} - Tiene luz 🟩\n"
                e["state"] = True
                print(f"{e} Arriba")
        else:
            if 'state' not in e or e["state"] != False:
                message = f"{name} - No tiene luz 🟥\n"
                e["state"] = False
                print(f"{e} Caido")
        return message
...
def buscar_equipo(message):
    global check_equipos

    command = message.text.split(' ', 1)

    if len(command) < 2:
        bot.reply_to(message, "Proporciona nombre para la busqueda.")
        return
    ...
    for e in check_equipos:
        print("***************************")
        print(command[1].lower() in e["name"].lower())
        print(command[1].lower(), e["name"].lower())

        if command[1].lower() in e["name"].lower():
            print("--------------------------")
            print(e)
            result = do_ping2(e)
